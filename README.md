...
국가전략기술 제1호 개정안의 **10대 분야 55개 중점기술**별로 OpenAlex · KCI에서 한국 논문을
검색하고, title + abstract를 근거로 주요 기술적 성과를 Gemini로 종합해 마크다운 보고서 + 통계 +
PDF로 내보내는 서비스. 검색식은 세부기술 단위로만 관리자가 등록하며, 대분류는 하위 결과를 모아
보여준다. 분야 단위로는 세부기술 보고서를 합성한 **종합 보고서**와, 등록된 로드맵과 대조하는
**이행 점검 보고서**를 따로 만든다.
...
curl -s localhost:8003/api/fields | head -c 300        # 10대 분야가 seed되어 있어야 함
```
...
2. **세부기술 등록**: 대분류 선택 → 이름 + 검색식(`query`, 필요 시 `query_kci` override) 입력.
   10대 분야와 55개 중점기술은 마이그레이션 `0010`으로 이미 seed되어 있으므로, 보통은 새로 만들기보다
   검색식을 손보는 작업이 된다. 검색식을 고칠 때는 `0010`을 편집하지 말고 새 마이그레이션에서 UPDATE한다.
...
6. **로드맵 등록**: 분야별로 로드맵 마크다운을 붙여넣는다. 이행 점검의 입력이므로 **표 형식**이어야 하고,
   표 본문 행이 하나도 없으면 저장이 422로 거부된다(줄글을 넣으면 전수 점검이 조용히 무력화되기 때문).
7. **분야 보고서**: "분야 보고서" 탭에서 종합 보고서 · 로드맵 이행 점검을 분야별로, 또는 당해연도 전체를
   일괄로 생성한다.

## 분야 단위 보고서 — 종합 · 로드맵 이행 점검

세부기술 파이프라인이 끝난 뒤, 그 결과(세부기술 보고서)를 입력으로 분야 단위 보고서 두 종류를 만든다.
LLM 1콜(약 10~17초)로 끝나지만 **동기 응답이 아니라 큐잉**한다 — "생성"을 누르면 행을 `pending`으로만
만들고 즉시 응답하며(화면은 그 자리에서 폴링), 실제 호출은 잡 루프가 **30초에 한 건씩** 처리한다.
수십 건을 일괄로 걸어도 Gemini API가 동시에 얻어맞지 않게 하기 위함이다.

| 보고서 | 내용 | 캐시 테이블 |
|---|---|---|
| **분야 종합** | 하위 세부기술 보고서를 합성한 분야 전체 성과 | `field_reports(field_id, year)` |
| **로드맵 이행 점검** | 등록된 로드맵의 목표를 **전수** 대조해 달성/데이터 없음/분석 범위 밖으로 판정 | `roadmap_checks(field_id, year)` |

- 재생성해도 기존 본문은 지우지 않고 `status`만 `pending`으로 되돌린다 — 처리가 끝날 때까지 이전
  보고서를 계속 보여주기 위해서다.
- 로드맵 이행 점검은 목표 행 수를 코드로 세어(`count_goal_rows`) 프롬프트에 박아 넣는다. 생성 후 다시
  세어 개수가 다르면 조회 응답에 `incomplete=true`가 붙어 화면에 경고가 뜬다(보고서는 남긴다).
- **논문 데이터의 원리적 한계**: 로드맵 후반 단계 목표는 대부분 양산성·가격 경쟁력·자급률이라 논문에
  실리지 않는다(실측: 1단계 확인 77% → 3단계 이상 22%). 그래서 보고서 자체에 "이 점검의 한계" 절을
  강제해, `데이터 없음`이 연구 부진을 뜻하지 않는다는 점을 밝히게 했다.
- **로드맵 원문은 Gemini API로 나간다** — 프롬프트에 그대로 실린다. 비공개 판본인지는 관리자가 판단한다.
- 전용 페이지(`/fields/{id}/report/{year}`)에서 "세부기술 보고서 포함"을 켜면 하위 보고서를 뒤에 이어붙여
  한 번에 인쇄(PDF)할 수 있다.
